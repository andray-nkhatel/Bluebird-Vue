<template>
    <div class="score-entry">
      <!-- Header -->
      <div class="flex justify-content-between align-items-center mb-4" style="align-content: center;">
        <div>
      ...
      <!-- Filters Card -->
      <Card class="mb-4" style="width: 500px;">
        <template #content>
          <div class="grid">
        ...
        <template #content>
          ...
          >
            ...
            <Column field="currentScore" header="Current Score" style="min-width: 120px;">
              <template #body="slotProps">
                <Tag
                  v-if="slotProps.data.currentScore !== null"
      ...
      <!-- Empty State -->
      <Card v-else-if="!loadingScores && selectedAssignment">
        <template #content>
          <div class="text-center py-6">
            ...
            <p class="text-500 mb-4">
              No students are enrolled in the selected grade or no scores exist for the selected criteria.
            </p>
            <Button
              label="Refresh"
              icon="pi pi-refresh"
  ...
  export default {
    ...
    methods: {
      async initializeData() {
        ...
        // Set current year and term if available
        if (this.academicYears.length > 0) {
          this.selectedAcademicYear = this.academicYears[0].year
        }
      ...
      async loadTeacherAssignments() {
        try {
          ...
          this.showError('Failed to load teacher assignments')
        } finally {
      ...
      async loadTerms() {
        try {
          ...
          this.terms = await examService.getTerms()
        } catch (error) {
      ...
      async loadStudentScores() {
        ...
        try {
          ...
          // Get all students for the grade (you might need a separate endpoint)
      ...
      onRowEditSave(event) {
        ...
        this.showSuccess('Score updated locally. Click "Save All Changes" to persist.')
      },
      ...
      async saveAllChanges() {
        ...
        try {
          ...
          if (changes.length === 1) {
            // Single score update
            await examService.submitScore(changes[0])
          } else {
            // Bulk score update
            await examService.bulkSubmitScores(changes)
          }
          ...
          this.showError('Failed to save scores')
        } finally {
          this.saving = false
        }
      },
