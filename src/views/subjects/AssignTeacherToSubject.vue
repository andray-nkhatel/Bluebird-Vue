...
export default {
  ...
  methods: {
    ...
    loadingAssignments(subjectId) {
      return subjectService.getSubjectAssignments(subjectId)
        .then(response => response.assignments || [])
        .catch(error => {
          console.error(`Failed to fetch assignments for subject ${subjectId}:`, error)
          return []
        })
    }
